...
export const productPurchaseOrders = pgTable(
  'product_purchase_orders',
  {
    ...
    code: text('code').unique().notNull(), // Format: PPO-000001
    vendorId: uuid('vendor_id')
      .notNull()
      .references(() => vendors.id),
    totalAmount: numeric('total_amount').notNull(), // Drived value
    completedAt: timestamp('completed_at', { withTimezone: true }), // Drived value
...
export const productPurchaseReceipts = pgTable(
  ...
  {
    id: uuid('id').defaultRandom().primaryKey(),
    code: text('code').unique().notNull(), // Format: PPR-000001
    productPurchaseOrderId: uuid('product_purchase_order_id')
      .notNull()
      .references(() => productPurchaseOrders.id),
